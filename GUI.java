...
class GUI extends TopLevel implements View, Reporter {
    ...
    GUI(String title) {
        ...
        addMenuButton("Controls->Set Move Limit", this::setMoveLimit);
        addSeparator("Controls");
        addMenuButton("Controls->Set White->Auto", this::setPlayer);
        addMenuButton("Controls->Set White->Manual", this::setPlayer);
        addMenuButton("Controls->Set Black->Auto", this::setPlayer);
        addMenuButton("Controls->Set Black->Manual", this::setPlayer);
        addMenuButton("LOA->Help", (s) -> displayText("Help LOA",
                HELP_TEXT));
        addMenuButton("LOA->About", (s) -> displayText("About LOA",
                ABOUT_TEXT));
        ...
        addLabel("To move: White", "CurrentTurn",
                 new LayoutSpec("x", 0, "y", 0,
                                "height", 1,
                                "width", 3));
        addLabel("White: Auto\t\tBlack: Manual", "manual/auto",
                new LayoutSpec("x", 0, "y", BoardWidget.BOARD_SIDE,
                "height", 1,
                "width", 3));
        addLabel("Moves made: 0\t\t\t\tMove limit: 30", "MovesMade",
                new LayoutSpec("x", 0, "y", BoardWidget.BOARD_SIDE + 1,
                "height", 1,
                "width", 3));
    }
    ...
    /** Response to "Undo" button - retracts two moves. */
    private void undo(String dummy) {
        _pendingCommands.offer("undo");
    }

    /** Response to "Set move limit" button.
     * Allows to set a different move limit. */
    private void setMoveLimit(String dummy) {
        String response =
                getTextInput("Enter move limit", "", "", "");
        if (response != null) {
            _pendingCommands.offer("limit " + response);
        }
    }

    /** Response to set Player for piece.
     * @param input the String the represents label for the button. */
    private void setPlayer(String input) {
        int piecei = input.indexOf(' ') + 1;
        int piecej = input.indexOf('-', piecei);
        int typei = input.indexOf('>', piecei) + 1;
        String piece = input.substring(piecei,
                piecej).toLowerCase();
        String type = input.substring(typei).toLowerCase();
        _pendingCommands.offer(type + " " + piece);
    }
    ...
    public void update(Game controller) {
        ...
        String whiteAM = "White: " + (manualWhite ? "Manual" : "Auto");
        String blackAM = "Black: " + (manualBlack ? "Manual" : "Auto");
        setLabel("manual/auto", whiteAM + "\t\t" + blackAM);
        setLabel("MovesMade", "Moves made: " + board.movesMade() / 2
                + "\t\t\t\t" + "Move limit: " + board.getMoveLimit() / 2);
    }
